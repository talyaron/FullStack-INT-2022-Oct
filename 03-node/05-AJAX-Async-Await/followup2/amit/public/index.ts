interface Flags{
    country: string;
    url: string;
}

const flags: Flags[] = [
    {
        country:"israel", url:"data:image/png;base64,"
    }
    {
        country:"colombia", url:"data:image/png;base64,"
    }
];

function handleGetFlags() {
    try {
      fetch("/api/get-flags")
        .then((res) => res.json())
        .then(({ flags }) => {
          try {
            if (!flags) throw new Error("didnt find users");
            renderFlags(flags);
          } catch (error) {
            console.error(error);
          }
        });
    } catch (error) {
      console.error(error);
    }
  }


function renderFlags(){
    try {
        const flagsRoot: HTMLDivElement | null = document.querySelector("#flagsRoot");

        if(!flagsRoot) throw new Error ("flags not found")

        flags.map(flag) => {
            return `<div class="userCard">
                <img src="${flag.src}" alt="user name is ${user.name}">
                <p>${user.name}</p>
                </div>`;
          })
          .join(" ");

    } catch (error) {
        console.error(error)
    }

}