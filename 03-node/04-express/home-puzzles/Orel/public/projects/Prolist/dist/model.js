...
function getLastRegisterUser() {
    try {
        ...
        var lastRegister = users[getUserIndexFromLocalStorage()];
        return lastRegister;
    }
    catch (error) {
        console.error(error);
...
function updateUsersToLocalStorage() {
    try {
        if (!users)
            throw new Error("not find Users in local storage");
...
function getUserIndexFromLocalStorage() {
    try {
        var stringUserIndex = localStorage.getItem("userIndex");
        if (!stringUserIndex)
            throw new Error("the userIndex not exist in the local storage");
        var userIndex = Number(stringUserIndex);
        return userIndex;
    }
    catch (error) {
        console.error(error);
...
function getLikesPhotos() {
    var filterLike = function (photo) { return photo.like; };
