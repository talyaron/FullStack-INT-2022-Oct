function newRegister(e) {
    try {
        ...
        const newUserInput = new userInput(id, name, address, email, phone, password, color, image);
...
function renderUserInputs(addUserInput) {
    try {
        if (userInputContainer){
            ...
            const userInputIndex = document.querySelector(`#userInput${addUserInput.id}`);
            if (userInputIndex){
