...
document.getElementById("highScore")?.innerHTML = "your high score is "+hiScore;
...
let drink_3 = new coctail("cosmopolitan", [bi_vodka, bi_cranberry,bi_lime,bi_ice,bi_tripleSec]);
let drink_4 = new coctail("Negroni",[bi_gin,bi_campari,bi_vermouth]);
let drink_5 = new coctail('Manhattan',[bi_whiskey,bi_vermouth,bi_Angostura,bi_ice])
let drink_6= new coctail('Old Fashioned',[bi_whiskey,bi_Angostura,bi_sugar,])
...
let drink_9= new coctail('French 75',[bi_gin,bi_syrup,bi_lemon,bi_champagne])
...
let drink_16 = new coctail('all',[bi_tequila,bi_rum,bi_champagne,bi_brandy])


for (var nickName in this) { if (nickName.startsWith("drink_")) {
...
function selectCocktail(){
   ...
   $("#hint").fadeOut(5000);
       ...
       document.getElementById("customerDialog")?.innerHTML = 'hello please give me  '+ selectedDrink.name;
   ...
   document.getElementById(targetShelf).append((document.getElementById("shaker")?.children[0]));
...
function serve(){
    ...
    if(shakerContent?.length != selectedDrink.ingredients.length){
        //loose
        document.getElementById("customerDialog")?.innerHTML='wft is this abomination , taste like shit';
        console.log("lose1");
        points=points-10
    }else{
        try{
        for(let i =0;i<shakerContent?.length;i++){
            if(selectedDrink.ingredients.findIndex(s => s.name === (shakerContent[i].id)) == -1){
                drinkTaste=false;
            }
        } 
        if(drinkTaste== true){
          //win
            document.getElementById("customerDialog")?.innerHTML='thank you, that was very tasty';
            points=points+5
            console.log("win");
        }else{
            //loose
            document.getElementById("customerDialog")?.innerHTML="give this cocktail to your manger";
            console.log("lose2");
            points=points-10
    ...
    if(points>hiScore){
        ...
        document.getElementById("highScore")?.innerHTML = `your highest score : ${points}`;
    }
    document.getElementById("points")?.innerHTML = `currnet score ${points}: `
    if(points<1){
        document.getElementById("customerDialog")?.innerHTML="YOU ARE FIRED , go home and learn some js";
    } else {
