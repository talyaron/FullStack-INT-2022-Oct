var popUpBalloons = function () {
    var density = 7;
    var balloons = [];
    var color = ["yellow", "green", "blue", "gold"];
    ...
    function releaseBalloon(balloon) {
        var delay = random(100, 1000);
        var x = random(-99, -30);
        var y = random(-99, -30);
        var order = [{ outside: 0, transform: "rotateZ(45deg) translate(0,0)" }];
        if (random(0, 1) === 0) {
        ...
        var balloonAnimation = balloon.animate(order, { duration: 15000, delay: delay });
        balloonAnimation.onfinish = function () {
            {
                releaseBalloon(balloons);
            }
            ;
