...
function logIn(): string | undefined {
    try {
        ...
        while (repeat === true) {
            const userName = prompt("Welcome to Amitflix! \n\nWhat is your user name?\n");
            if ((userName !== null)) {
...
function removeVideoFromWatchlist(videoName: string): void {
    try {
        ...
        const index = user.videosOrdered.findIndex(video => video.name === videoName);
...
function showSlides(): void {
    try {
        let slides = document.getElementsByClassName("videoSlides") as HTMLCollectionOf<HTMLElement>;
...
function validateNewVideoOnWatchlist(newVideo: Video, user: User): void {
    const index = user.videosOrdered.findIndex(video => video.name === newVideo.name);
    if (index !== undefined && index > -1) {
