...
const Balloon: FC<BalloonProps> = ({setBalloons, balloons, balloon})=>{
    function handleClickBalloon(balloonId: string){
        try {
            const balloonIndex = balloons.findIndex((balloon)=>balloon._id===balloonId);
            if(balloonIndex === -1) throw new Error("balloon no found");
    ...
    return(
        <div>
            <img onClick={()=>handleClickBalloon(balloon._id)}
            className="balloon" key={balloon._id} src={balloon.image}
            alt="balloon"
            />
        </div>
    )
